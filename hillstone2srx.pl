...
sub set_compare {
    ...
    $worksheet->write( $row, 0, $hillstone_conf );
    $worksheet->write( $row, 1, $srx_conf );
    $row++;
}

sub set_address_books {
    local $address_book_name = "@_";
    local @hillstone_config;
    local @srx_config;
...
if (defined $opt_c) {
    ...
    $hilston_format = $workbook->add_format();
    $srx_format = $workbook->add_format();
    $hilston_format->set_color( 'green' );
    $hilston_format->set_align( 'left' );
